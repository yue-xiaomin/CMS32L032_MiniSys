...
//-----------------------------------------------------------------------   
	KEY_Init(1 << 0);                       // Initializes P32/KR0
...
//----------------------------------------------------------------------- 
	...
	GPIO_Set_Value(&PORT->P3, 0x00);        // LED OFF: P30, P31 output low Level
	delayMS(500);                           // LED ON: 500ms for reconnect debugger
	GPIO_Set_Value(&PORT->P3, 0x03);        // LED OFF: P31, P30 output High Level	
...
//-----------------------------------------------------------------------
// Sleep 4 times
// P32/KR0 falling edge wakeup system and blinky P30 LED
//-----------------------------------------------------------------------
	for(i=4; i>0; )
	{
		__WFI(); 			// Sleep
		if(0U == (PORT->P3 & 0x01)) // check P70 value is 0
		{
			PORT->P3 ^= (1 << 2); 	// P31 toggle 
			i--;
		}
	}
