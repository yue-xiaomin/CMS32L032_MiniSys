...
MD_STATUS SPI10_MasterSendReceive(uint8_t *const tx_buf, uint16_t tx_num, uint8_t *const rx_buf)
{
    ...
    else
    {
        ...
        DMAVEC->CTRL[CTRL_DATA_SPI10].DMSAR = (uint32_t)&SCI1->SIO10;
        ...
        DMAVEC->CTRL[CTRL_DATA_SPI10 + 1].DMDAR = (uint32_t)&SCI1->SIO10;
        ...
        SPI10_Start();
        SCI1->SIO10 = *gp_spi10_tx_address; /* started by writing data to SDR[7:0] */
        gp_spi10_tx_address++;
        g_spi10_tx_count--;
    }
...
MD_STATUS SPI10_MasterSend(uint8_t *const tx_buf, uint16_t tx_num)
{
    ...
    else
    {
        ...
        DMAVEC->CTRL[CTRL_DATA_SPI10].DMDAR = (uint32_t)&SCI1->SIO10;
...
MD_STATUS SPI10_SlaveSend(uint8_t *const tx_buf, uint16_t tx_num)
{
    ...
    else
    {
        ...
        gp_spi10_tx_address++;
        g_spi10_tx_count--;
        SCI1->SIO10 = tx_buf[0]; /* started by writing data to SDR[7:0] */
    }
...
void SPI11_MasterInit(spi_mode_t mode)
{
    ...
    SCI1->SCR11 = _0004_SCRMN_DEFAULT_VALUE | _C000_SCI_RECEPTION_TRANSMISSION | dap << 13 | ckp << 12 | _0000_SCI_INTSRE_MASK |
                  _0000_SCI_PARITY_NONE | _0000_SCI_MSB | _0000_SCI_STOP_NONE | _0003_SCI_LENGTH_8;
  //SCI1->SDR03 = 0 << 9; 	// fMCK/(1+n)x2 = fMCK/2
  //SCI1->SDR03 = 1 << 9; 	// fMCK/(1+n)x2 = fMCK/4
  //SCI1->SDR03 = 2 << 9;		// fMCK/(1+n)x2 = fMCK/6
  SCI1->SDR11 = 3 << 9;		// fMCK/(1+n)x2 = fMCK/8  
	//SCI1->SDR03 = 5 << 9;		// fMCK/(1+n)x2 = fMCK/12
	//SCI1->SDR03 = 7 << 9;		// fMCK/(1+n)x2 = fMCK/16
	//SCI1->SDR03 = 15 << 9;	// fMCK/(1+n)x2 = fMCK/32
	//SCI1->SDR03 = _CE00_SCI_BAUDRATE_DIVISOR;
    /* Set output enable */
    if ((mode == SPI_MODE_0) || (mode == SPI_MODE_1))
    {
