...
// P31/LED indicates the result of data received by mode 3
//-----------------------------------------------------------------------
#ifdef USE_HANDSHAKE
	INTP0_PORT_SETTING();
	PORT->PM1 &= ~(1<<0);  // P136 output mode
#endif
	PORT->PMC3 &= ~(3<<0);  // P31, P30 digital function
	PORT->PM3  &= ~(3<<0);  // P31, P30 output mode
	PORT->P3   |=  (3<<0);  // P31/LED, P30/LED OFF
...
#ifdef USE_HANDSHAKE
	/* P10 output a pulse to flag SPI slave ready */
	PORT->PSET1 = (1<<0);
	for(i=0; i<10; i++);
	PORT->PCLR1 = (1<<0);
#endif
